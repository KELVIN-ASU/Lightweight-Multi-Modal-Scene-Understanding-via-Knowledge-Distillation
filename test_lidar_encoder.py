...
def main():
    ...
    cam_enc = TwinLiteEncoder(return_multiscale=True).to(device)
    lidar_enc = LiDAREncoder(encoder_type="spatial", grid_size=(64,64), use_vectorized=True).to(device)
    ...
    # Test x4 mode
    # --------------------
    model_x4 = CompleteSegmentationModel(
        cam_enc, lidar_enc,
        num_classes=3,
        fusion_type="concat",
    ...
    with torch.no_grad():
        logits_x4 = model_x4(images, points)
    print("x4 mode output:", logits_x4.shape)  # expect [B,3,256,256]

    # --------------------
    # Test same mode
    # --------------------
    model_same = CompleteSegmentationModel(
        cam_enc, lidar_enc,
        num_classes=3,
        fusion_type="concat",
        fusion_out_channels=256,
    ...
    with torch.no_grad():
        logits_same = model_same(images, points)
    print("same mode output:", logits_same.shape)  # expect [B,3,64,64]
